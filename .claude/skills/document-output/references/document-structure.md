# Document Structure

## Template

```markdown
# [Descriptive Title]

**Generated:** [Date]
**Context:** [Brief description of what was requested]

---

[Main content with proper formatting — headers, lists, tables]

---

## Notes

[Any additional context, caveats, or follow-up suggestions]
```

## Informing the User

After creating the document:

- Tell the user a document was created
- Provide the file path as a markdown link
- Give a brief summary (2–3 sentences) of what's in it
- Ask if they want modifications or additional information

## Worked Examples

### "Give me 10 blog post ideas about testing"

1. Create `blog/brainstorms/testing-blog-ideas.md`
2. Include all 10 ideas with descriptions, SEO notes, and priority indicators
3. Respond: "I've created a document with 10 testing blog post ideas at [blog/brainstorms/testing-blog-ideas.md](blog/brainstorms/testing-blog-ideas.md). The ideas range from beginner topics like mocking basics to advanced concepts like mutation testing. Want me to expand on any of these or add SEO research?"

### "Compare NSubstitute vs Moq"

1. Create `research/comparisons/nsubstitute-vs-moq.md`
2. Include a feature comparison table, pros/cons, use cases, and code examples
3. Respond with a summary and the file location

### "What are some good testing conferences?"

1. Create `research/testing-conferences.md`
2. Include conference names, dates, locations, CFP deadlines, focus areas
3. Respond with a summary and the file location
