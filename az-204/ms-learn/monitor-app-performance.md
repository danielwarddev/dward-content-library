...
Since _standard metrics_ are preaggregated during collection, they have better performance at query time. Standard metrics are a better choice for dashboarding and in real-time alerting. The _log-based metrics_ have more dimensions, which makes them the superior option for data analysis and ad-hoc diagnostics. Use the namespace selector to switch between log-based and standard metrics in metrics explorer.
...
> **Important**
>
> Both, log-based and pre-aggregated metrics coexist in Application Insights. To differentiate the two, in the Application Insights UX the pre-aggregated metrics are now called _Standard metrics (preview)_, while the traditional metrics from the events were renamed to _Log-based metrics_.
...
> **Note:** This unit contains a hands-on lab exercise. The exercise involves creating an Azure App Service web app with Application Insights enabled, configuring autoinstrumentation without modifying code, creating and deploying a Blazor application, and viewing application metrics and error data in Application Insights.
>
