...
Azure API Management is made up of an _API gateway_, a _management plane_, and a _developer portal_. These components are Azure-hosted and fully managed by default. API Management is available in various tiers differing in capacity and features.
...
```xml
<retry condition="boolean expression or literal" count="number of retry attempts" interval="retry interval in seconds" max-interval="maximum retry interval in seconds" delta="retry interval delta in seconds" first-fast-retry="boolean expression or literal">
...
> **Note:** This unit contains a hands-on exercise. The exercise involves creating an Azure API Management instance, importing an OpenAPI specification backend API, configuring the API settings, and testing the API operations.
...
This exercise takes approximately **20** minutes to complete and requires an Azure subscription.
