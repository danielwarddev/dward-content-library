...
> **Note:** Event Grid doesn't guarantee order for event delivery, so subscribers might receive them out of order.
...
> **Important:** If Dead-Letter isn't configured for an endpoint, events are dropped when the above errors happen. Consider configuring Dead-Letter if you don't want these kinds of events to be dropped.
...
> **Note:** This unit contains a hands-on lab exercise. The exercise requires an Azure subscription and involves creating Event Grid resources, setting up event subscriptions, and verifying that your endpoint receives events from Azure services.
